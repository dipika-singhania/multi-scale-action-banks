...
new_img_tmp = 'img_{:01d}.jpg'

write_env = lmdb.open('/mnt/data/epic_kitchen/new_anticipation_model/' + modality + '_temp/', map_size=1099511627776)
read_env = lmdb.open('/mnt/data/epic_kitchen/new_anticipation_model/' + modality + '/')

csv_file = pd.read_csv(modality + '_video_frames_info.csv', header=None, names=['video','start','end'], dtype={'start':np.int64, 'end':np.int64})
